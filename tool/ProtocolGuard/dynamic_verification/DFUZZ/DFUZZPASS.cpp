...
namespace {
  ...
  struct DFUZZPASS: public ModulePass {
    ...
    bool runOnModule(Module & M) {
      ...
      int total_BB=0;
      std::ofstream cfg_ddg, debug, distance;
       
       // 安全地打开文件流
      cfg_ddg.open(OutDirectory + "/ctrl-data.dot", std::ofstream::out | std::ofstream::app);
      debug.open(OutDirectory + "/debug.txt", std::ofstream::out | std::ofstream::app);
      distance.open(OutDirectory + "/distance.cfg.txt", std::ofstream::out | std::ofstream::app);
      
      // 检查文件是否成功打开
      if (!cfg_ddg.is_open() || !debug.is_open() || !distance.is_open()) {
      ...
      */
      ...
      for (auto bb: rel_bb) {
        ...
        if (map_bb.count(bb) == 1){
          ...
          if(pro_bb_map.count(bb)==1){
            ...
            errs() << "BB: " << map_bb.at(bb) << " distance: " << pro_bb_map[bb] << "\n";
          }
        }
      }
      ...
      if (cfg_ddg.is_open()) cfg_ddg.close();
      if (debug.is_open()) debug.close();
      if (distance.is_open()) distance.close();
