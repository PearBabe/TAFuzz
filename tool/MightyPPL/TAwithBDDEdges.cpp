...
//                            std::cout << "(" << l1 << ", " << l2 << ") syncs" << std::endl;
    ...
    TAwithBDDEdges TAwithBDDEdges::intersection(const std::vector<TAwithBDDEdges>& components) {
        ...
        if (mightypplcpp::back) {
            ...
            auto initial_state = monitaal::symbolic_state_t(projected.initial_location(), projected.number_of_clocks());
            monitaal::symbolic_state_map_t<monitaal::symbolic_state_t> reachable_state_space;
            if (mightypplcpp::out_fin) {

                // Note that we assume the finite timed word accepted is of length >= 1 (as enforced by the acc. condition of TA_0)
                reachable_state_space = monitaal::Fixpoint<monitaal::symbolic_state_t>::reach(monitaal::Fixpoint<monitaal::symbolic_state_t>::accept_states(projected), projected); 
                ...
                reachable_state_space = monitaal::Fixpoint<monitaal::symbolic_state_t>::buchi_accept_fixpoint(projected);
                if (initial_state.is_included_in(reachable_state_space)) {
            ...
            for (const auto& [id, s] : reachable_state_space) {
                ...
                reachable_locations_timed.emplace(id, reachable_locations.at(id));
