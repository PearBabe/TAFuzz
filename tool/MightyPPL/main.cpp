...
//    }
    ...
    if (!out_format.has_value()) {
        ...
        auto initial_state = monitaal::symbolic_state_t(pos.initial_location(), pos.number_of_clocks());

        if (out_fin) {

            // Note that we assume the finite timed word accepted is of length >= 1 (also enforced by the acc. condition of TA_0)
            if (initial_state.is_included_in(monitaal::Fixpoint<monitaal::symbolic_state_t>::reach(monitaal::Fixpoint<monitaal::symbolic_state_t>::accept_states(pos), pos))) {
            ...
            if (initial_state.is_included_in(monitaal::Fixpoint<monitaal::symbolic_state_t>::buchi_accept_fixpoint(pos))) {
