...
- `SIM_BATT_VOLTAGE` / `11.55`：`CONFIRMED_EFFECT`；确认状态=['status']；旧规则状态=['altitude']。
- `RC1` / `1700`：`CONFIRMED_EFFECT`；确认状态=['roll']；旧规则状态=['roll', 'yaw']；协议字段=`RC_CHANNELS_OVERRIDE.chan1_raw`。
- `FS_GCS_TIMEOUT` / `2.0`：`CONFIRMED_EFFECT`；确认状态=['status']；旧规则状态=[]。
